function [results,my] = tracker_test_flops(params)
...
%             return;
    ...
    if flag_seg_sp && seq.frame==401 && (sum(my.score(1:seq.frame,2)>0.15)>0.5*seq.frame || numGoodIouMotion<5)
        flag_seg_sp = false;
%         results.res = seq.rect_position(1:400,:);
%         return;
    end
...
%     if seq.frame>1 && gmc_success && isempty(gmc_pos)
%         flag_detect = false;
%         flag_update = false;
...
%     end
    ...
    if seq.frame > 1
        ...
        if flag_detect
            ...
            while iter <= params.refinement_iterations && any(old_pos ~= pos)
                ...
                my.tmp_time(seq.frame) = my.tmp_time(seq.frame)+toc();
                tmp_time = my.tmp_time(seq.frame);
                if ~isempty(gmc_pos)
                    my.tmp_time(seq.frame) = my.tmp_time(seq.frame)+tmp_time;
                end
                if ~isempty(dx_lf)
                    my.tmp_time(seq.frame) = my.tmp_time(seq.frame)+tmp_time;
                end
    ...
    if seq.frame>1 && gmc_success
        ...
        %flag_ctr = norm(tmp-size(im,[1 2])/2)<0.2*min(size(im,[1 2]));
    ...
    if seq.frame>1
        ...
        switch signal
            ...
            case '100'
                gmc_success;
                %disp('take the only one');
            case '000'
                %disp('take the baseline');
    ...
    if (flag_seg || flag_seg_small) && ~isempty(pred_seg)&& pred_seg(3)>0 && pred_seg(4)>0 && cf_sim_score<0.2
        ...
        if flag_seg || flag_seg_small
            ...
            if color_hist_score2<color_hist_score1+0.1 && color_hist_score2<0.2 || flag_seg_very_stable
                ...
                %if seq.frame>5
                %    fprintf('std1: %.2f, std2: %.2f\n',std(my.seg_box(seq.frame-5:seq.frame,3)),std(my.seg_box(seq.frame-5:seq.frame,3)));
                %end
...
%             strong_update = true;
%         end
%         pos = [clf_bb(2) clf_bb(1)];
%         target_sz = [clf_bb(4) clf_bb(3)];
%         currentScaleFactor = sqrt(prod(target_sz*params.search_area_scale)/prod(img_sample_sz));
%         
%         if nScales > 0
%             %force reasonable scale changes
...
%         
%         base_target_sz = target_sz/currentScaleFactor;
%         %disp(['frame ' num2str(seq.frame-1) ' use clf']);
%     end
    ...
    if seq.frame>10 && flag_seg_sp && ~flag_occ && gmc_success && cf_sim_score<0.2
       % try superpixel seg
...
disp(['numGoodIouMotion: ' num2str(numGoodIouMotion)]);