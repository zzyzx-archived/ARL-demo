...
def floodprint(mat, pos, old, new, connection_type):
    ...
    while len(stack) > 0:
        ...
        if connection_type == 4:
            ...
            neighbors = ((i - 1, j - 1), (i - 1, j), (i, j - 1), (i + 1, j + 1), (i + 1, j), (i, j + 1), (i - 1, j + 1),
                         (i + 1, j - 1))
...
def isInBox(ptRc, bbox):
    ...
    return (ymin <= ptRc[0] <= ymax) and (xmin <= ptRc[1] <= xmax)
...
def show_track(img, track_bb, color=(0, 0, 255)):
    if track_bb is None or len(track_bb) < 1:
        return img
    plt_bb = np.array(track_bb).astype(np.int32)
    # print(plt_bb)
    img_t = cv2.rectangle(img.copy(), (plt_bb[0], plt_bb[1]), (plt_bb[0] + plt_bb[2], plt_bb[1] + plt_bb[3]), color, 2)
    return img_t
...
def get_subwindow_avg(im, pos, model_sz, original_sz, fill_color=None):
    ...
    if top_pad > 0 or bottom_pad > 0 or left_pad > 0 or right_pad > 0:
        R = np.pad(im[:, :, 0], ((top_pad, bottom_pad), (left_pad, right_pad)), 'constant',
                   constant_values=(avg_chans[0]))
        G = np.pad(im[:, :, 1], ((top_pad, bottom_pad), (left_pad, right_pad)), 'constant',
                   constant_values=(avg_chans[1]))
        B = np.pad(im[:, :, 2], ((top_pad, bottom_pad), (left_pad, right_pad)), 'constant',
...
def get_crops(im, bbox, size_z, size_x, context_amount, factor=1, fill_color=None):
    ...
    image_crop_x, _, _, _, _ = get_subwindow_avg(im, [cy, cx], [size_x, size_x], [np.round(s_x), np.round(s_x)],
...
def get_main_color(patch, feat, bb, num_color=5, ct_dist=None, verbose=False):
    ...
    mid_salient_score = getScoreInBox(tmp,
                                      convert_bbox_format([bb_ct[0], bb_ct[1], bb_ct[2] / 2, bb_ct[3] / 2], 'topleft'))
    ...
    if salient_iou < 0.5 or salient_iou_big < 0.5 or mid_salient_score < 0.3 or salient_dif < 30:
        print('salient_iou,salient_iou_big,mid_salient_score,salient_dif:', salient_iou, salient_iou_big,
              mid_salient_score, salient_dif)
        salient_set = []
...
from myutils.features.features import TableFeature
...
def init_seg_params(img_path, bb, mode='pdf', cts=None, verbose=False):
    ...
    if mode == 'pdf':
        ...
        init_cts = np.array([np.mean(init_patch[tmp == v], axis=0) if np.sum(tmp == v) > 0 else [0, 0, 0] for v in
                             range(init_cn_feat.shape[-1])])
        ...
        salient_set, hist_dif_wei = get_main_color(init_patch, init_cn_feat, init_bb,
                                                   num_color=num_color, ct_dist=euclidean_distances(init_cts),
                                                   verbose=verbose)
    else:
...
def get_coords_prior(patch, bb, salient_set, hist, fnum, bnum, gmm_cts=None, mode='pdf', verbose=False):
    ...
    bg_coords = np.array([v for v in full if \
                          not (xmin <= v[1] <= xmax and ymin <= v[0] <= ymax) and not (
                                      cn_feat[v[0]][v[1]] in salient_set) \
...
def process_frame(img_path, bb, info, mode='pdf', verbose=False):
    ...
    flag_normal, fg_coords, bg_coords, salient_scores = get_coords_prior(patch, bb_scaled, salient_set, hist_dif_wei,
                                                                         fnum=100, bnum=100, gmm_cts=init_cts,
