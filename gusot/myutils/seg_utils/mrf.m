...
function out = update_data_loss(data_loss, pixels, row_coors, col_coors, gmms)
out = data_loss;
for i=1:2
    for j=1:size(pixels,1)
        %[~,ds] = posterior(gmms{i},pixels(j,:));
        ds = gmm_score_samples(gmms{i},pixels(j,:));
        out(row_coors(j), col_coors(j), i) = ds;
    end
end
