function [pred_bb_last,baseline_pred,pred_bb,cf_sim_score,results,my] = run_GUSOT_consec(benchmark,frame_id, video_path, clf_bb, flag_must_use_clf, total_num_frame, save_name)
%run_GUSOT_consec Summary of this function goes here
...
if strcmp(benchmark,'LaSOT')
    ...
    seq.frames = cellstr(img_files);
else
    video_path = ['/home/zhiruo/Documents/VOT/code/1030_opti/seq_info/' benchmark '/' video_path '.txt'];
    data = readlines(video_path);
    seq.frames = cellstr(data(2:end-1));
end

display(['dive in at frame ',num2str(seq.frame)]);
%% 
while true
    seq.frame = seq.frame+1;
    frame_path =seq.frames{seq.frame};
    im = imread(seq.frames{seq.frame});
    ...
    if seq.frame==1
        ...
        flag.flag_no_refine = true; % can set to true on linux
    ...
    if flag_must_use_clf
        ...
        flag_must_use_clf = false;
        flag.flag_no_refine = false;
    end
