...
class GotTrack:
    ...
    def init(self, frame, init_bb, max_length=2000, add_self_gmc=False):
        ...
        self.kcf = KCF()
        self.kcf.init(img, init_bb)
        self.bs_bb = None
        self.bs_bb_last = np.array(init_bb)
    ...
    def update(self, frame):
        ...
        # global branch
        self.bs_bb = self.kcf.update(img)
        cf_sim_score = self.kcf.max_response
        ...
        if not self.flag_slim:
            ...
            bs_scaled = getScaledBox(self.bs_bb_last, self.bs_bb, viz_data[3], viz_data[0].shape[:2])
            if len(clf_bbt) > 0:
            ...
            not self.flag_bad_clf):
                flag_mrf = False
                if np.sum(tmp > 0.7) < 3:
                    flag_normal, fg_coords, bg_coords = get_coords_prior(patch.shape[:2], bs_scaled,
            ...
            if self.add_self_gmc:
                gmc_pred = self.get_motion(img)
                tmp = getScoreInBox(prob_n, bs_scaled)
                flag_loss_obj = tmp < 0.2 and len(clf_bbt) < 1 or tmp < 0.1
                if self.flag_bad_clf and flag_loss_obj and len(gmc_pred) > 0:
                    gmc_sim_score = min(0, cf_sim_score)

                    xy_pred = self.final_pred[:2]
        ...
        # update base tracker template shape
        if self.flag_must_use_clf:
            self.kcf.update_template(img, self.bb2ml)
